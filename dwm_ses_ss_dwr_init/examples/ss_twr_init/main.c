...
int main(void)
{
  ...
  /* Used for antenna delay calibrations */
  uint32_t ant_delay = 0;
  dwt_otpread(0x01c, &ant_delay, 1);
  printf("Ant_delay uint16: %hu \n\n\n", (uint16_t)ant_delay);
  ...
  if(ant_delay == 0){
    ...
    /* Apply OTP antenna delay value. */
    dwt_setrxantennadelay((uint16_t)ant_delay);
    dwt_settxantennadelay((uint16_t)ant_delay);
  }  
